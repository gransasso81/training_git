...
def make_matrix_X(x, p):
    n = len(x)
    # [x_n, ..., x_1, 0, ..., 0]
    xz = np.concatenate([x[::-1], np.zeros(p)]) #reverse x and append p zeros at the end
    X = np.zeros((n-1 , p))
    for i in range(n - 1):
        offset = n - 1 - i
...
[sample_rate, amplitudes] = scipy.io.wavfile.read(r'C:\Users\Andrea\Desktop\LPC\speech.wav')
...
l_w = 0.03
sym = False # periodic
w = hann(floor(l_w*sample_rate), sym) 
